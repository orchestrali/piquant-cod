...
const keysP = ['firstTowerbellPeal', 'firstHandbellPeal', 'firstTowerbellExtent', 'firstHandbellExtent']
...
module.exports = function handleMethod(method, tokens) {
  
  for (var i = 0; i < tokens.length; i++) {
    if (tokens[i].name === "title") {
      let names = newnames.find(o => o.newtitle == tokens[i].value);
      if (names) {
        method.oldtitle = names.oldtitle;
      }
    }
    ...
    if (text.indexOf(tokens[i].name) > -1) {
      ...
      for (var j = 0; j < perfs.length; j++) {
        ...
        pArr.push(handlePerform(p, pTokens));
      }
