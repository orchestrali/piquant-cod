...
module.exports = function handlePerform(p, tokens) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (tokens[i].name == 'society') {
      ...
      if (tokens[i].value.substring(0,2) != 'bb') return console.log('not a bellboard ID: ' + tokens[i].value);
    ...
    if (discarded.length > 0) {
      console.log('performance discarded keys: ' + discarded);
    }
  }
